...
class SessionAbandonmentDataset(Dataset):
    ...
    def __init__(
        ...
        # Build action type vocabulary from data
        self._build_action_type_vocab()

        # Pre-compute all S-BERT embeddings in batch (much faster than one-by-one)
    ...
    def _precompute_all_embeddings(self):
        """Batch-encode all unique texts with S-BERT upfront.

        This is ~10-50x faster than encoding one text at a time during
        training because S-BERT can batch GPU/CPU operations.
        """
        import json as json_lib

        # Collect unique (action_type, text) pairs
        unique_texts = {}  # cache_key -> text
        for session_df in self.sessions.values():
            for _, row in session_df.head(self.max_seq_len).iterrows():
        ...
        # Separate texts that need encoding from empty ones
        keys_to_encode = []
        texts_to_encode = []
        for key, text in unique_texts.items():
            if text:
                keys_to_encode.append(key)
                texts_to_encode.append(text)
            else:
                self.embedding_cache[key] = np.zeros(self.embedding_dim)

        if texts_to_encode:
            print(f"Pre-computing {len(texts_to_encode)} unique S-BERT embeddings...")
            embeddings = self.sbert_model.encode(
                texts_to_encode, convert_to_numpy=True,
                batch_size=256, show_progress_bar=True
            )
            for key, emb in zip(keys_to_encode, embeddings):
                self.embedding_cache[key] = emb
            print(f"Pre-computation complete. Cache size: {len(self.embedding_cache)}")
    ...
    def _get_content_text(self, row: pd.Series) -> str:
        ...
        if isinstance(content, str):
            ...
            try:
                ...
                elif 'movie_title' in content_dict:
                    # MovieLens: use movie title + genres
                    parts = [content_dict['movie_title']]
                    if 'genres' in content_dict:
                        parts.append(content_dict['genres'])
                    return ' '.join(parts)
            ...
            # Return as is (handles SO plain text content)
            return content
        
        return str(content)
    
    def _build_action_type_vocab(self):
        """Build action type vocabulary dynamically from the data."""
        unique_types = sorted(self.df['action_type'].dropna().unique())
        self.action_type_map = {t: i for i, t in enumerate(unique_types)}
        self.action_type_pad = len(self.action_type_map)
        self.num_action_types = self.action_type_pad + 1  # +1 for PAD

    def _compute_event_embedding(self, row: pd.Series) -> np.ndarray:
        """
        Compute S-BERT embedding for an event.

        All event types (QUERY, CLICK, SERP_VIEW) are encoded via S-BERT.
        """
        # Get text content
        text = self._get_content_text(row)
    ...
    def _process_session(self, session_df: pd.DataFrame) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            Dictionary with:
            - event_embeddings: (seq_len, embedding_dim)
            - action_type_ids: (seq_len,)
        ...
        # Compute event embeddings and action type IDs
        embeddings = []
        action_type_ids = []
        for _, row in session_df.iterrows():
            emb = self._compute_event_embedding(row)
            embeddings.append(emb)
            action_type_ids.append(
                self.action_type_map.get(row['action_type'], self.action_type_pad)
            )

        embeddings = np.array(embeddings)  # (seq_len, embedding_dim)
        ...
        result = {
            ...
            'action_type_ids': torch.LongTensor(action_type_ids),
...
def collate_fn(batch: List[Dict[str, torch.Tensor]], action_type_pad: int = 3) -> Dict[str, torch.Tensor]:
    """
    ...
    # Pad action_type_ids with the dataset-specific PAD index
    action_type_ids = torch.full((batch_size, max_len), action_type_pad, dtype=torch.long)
    ...
    for i, item in enumerate(batch):
        ...
        action_type_ids[i, :seq_len] = item['action_type_ids']
...
def create_dataloaders(
    ...
    # Build unified action type vocabulary across all splits
    all_action_types = set()
    for ds in [train_dataset, val_dataset, test_dataset]:
        all_action_types |= set(ds.action_type_map.keys())
    unified_action_map = {t: i for i, t in enumerate(sorted(all_action_types))}
    unified_action_pad = len(unified_action_map)
    num_action_types = unified_action_pad + 1  # +1 for PAD

    for ds in [train_dataset, val_dataset, test_dataset]:
        ds.action_type_map = unified_action_map
        ds.action_type_pad = unified_action_pad
        ds.num_action_types = num_action_types

    print(f"Action types ({len(unified_action_map)}): {sorted(unified_action_map.keys())}")

    # Create collate function with correct pad value
    collate = lambda batch: collate_fn(batch, action_type_pad=unified_action_pad)
    ...
    # Use a unified vocabulary across all splits to avoid index-out-of-range errors
    num_cognitive_labels = None
    if include_cognitive:
        # Build unified vocab from all splits
        all_labels = set(train_dataset.cognitive_label_to_id.keys())
        all_labels |= set(val_dataset.cognitive_label_to_id.keys())
        all_labels |= set(test_dataset.cognitive_label_to_id.keys())
        all_labels.discard('<UNK>')

        unified_label_to_id = {label: idx for idx, label in enumerate(sorted(all_labels))}
        unified_label_to_id['<UNK>'] = len(unified_label_to_id)
        unified_id_to_label = {v: k for k, v in unified_label_to_id.items()}

        # Apply unified vocab to all splits
        for ds in [train_dataset, val_dataset, test_dataset]:
            ds.cognitive_label_to_id = unified_label_to_id
            ds.cognitive_id_to_label = unified_id_to_label

        num_cognitive_labels = len(unified_label_to_id)
        print(f"Number of cognitive labels: {num_cognitive_labels}")

    return train_loader, val_loader, test_loader, num_cognitive_labels, num_action_types
