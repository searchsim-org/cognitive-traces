...
class Trainer:
    ...
    def train_epoch(self) -> float:
        ...
        for batch in pbar:
            ...
            action_type_ids = batch['action_type_ids'].to(self.device)
            ...
            if 'cognitive_label_ids' in batch:
                ...
                logits = self.model(event_embeddings, action_type_ids, cognitive_ids, attention_mask)
            else:
                logits = self.model(event_embeddings, action_type_ids, attention_mask)
    ...
    def save_checkpoint(self, epoch: int, metrics: Dict[str, float], is_best: bool = False):
        """Save model checkpoint (only saves best checkpoint to save disk space)."""
        if not is_best:
            return
...
def train_behavioral_model(
    ...
    # Load data
    train_loader, val_loader, test_loader, _, num_action_types = create_dataloaders(
        data_dir=data_dir,
...
def train_cognitive_model(
    ...
    # Load data
    train_loader, val_loader, test_loader, num_cognitive_labels, num_action_types = create_dataloaders(
        data_dir=data_dir,
        batch_size=batch_size,
...
def train_shuffled_model(
    ...
    """
    Train shuffled-label ablation model.
    ...
    print("="*60)
    print("TRAINING SHUFFLED-LABEL ABLATION MODEL")
    print("="*60)

    # Load data (with cognitive labels — they will be shuffled in forward())
    train_loader, val_loader, test_loader, num_cognitive_labels, num_action_types = create_dataloaders(
        data_dir=data_dir,
        batch_size=batch_size,
        include_cognitive=True
...
def main():
    ...
    parser.add_argument('--model-type', type=str,
                       choices=['behavioral', 'cognitive', 'shuffled', 'both', 'all'],
    ...
    if args.model_type in ['behavioral', 'both', 'all']:
    ...
    if args.model_type in ['cognitive', 'both', 'all']:
    ...
    if args.model_type in ['shuffled', 'all']:
        shuffled_f1 = train_shuffled_model(
            data_dir=args.data_dir,
            output_dir=args.output_dir,
            num_epochs=args.num_epochs,
            batch_size=args.batch_size,
    ...
    if 'behavioral' in results and 'cognitive' in results:
        improvement = (results['cognitive'] - results['behavioral']) / max(results['behavioral'], 1e-10) * 100
        print(f"\nCognitive vs Behavioral: {improvement:+.1f}%")
    if 'behavioral' in results and 'shuffled' in results:
        improvement = (results['shuffled'] - results['behavioral']) / max(results['behavioral'], 1e-10) * 100
        print(f"Shuffled vs Behavioral: {improvement:+.1f}%")
